...
function cleaner(opts) {
  ...
  /**
   * Sets options on instance of `connect`
   * Expects Object or key/value pair
   * @param {Object, String} key   Object containig group of options
   * or String - key of option
   * @param {[String]} value Value of option
   */
  middleware.set = function get(key, value) {
    if (isType(key, 'object')) {
      key.keys.forEach(function(key) {
      ...
      throw new Error('No such key in options');
    }
  ...
  /**
   * Expects optional `key` arguments
   * Returns full options object or option if `key` is set
   * @param  {[String]} key Name of need option, optional
   * @return {[type]}     Options object or value of `key` argument it if set
   */
